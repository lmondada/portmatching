...
use portmatching::{
    matrix::{MatrixIndexKey, MatrixManyMatcher, MatrixPattern, MatrixString},
    portgraph::{indexing::PGIndexKey, PGManyPatternMatcher, PGPattern},
    string::{StringIndexKey, StringManyMatcher, StringPattern},
...
fn string_matching() {
    ...
    print_matches(&matches, &StringIndexKey::root());
...
"#,
    ...
    let matches = matcher.find_matches(&text).collect_vec();
    print_matches(&matches, &MatrixIndexKey::root());
...
fn print_matches<K, V: Debug, M: IndexMap<K, V>>(matches: &[PatternMatch<M>], root_pos: &K) {
    for m in matches {
        println!(
            "Pattern {} matches at {:?}",
            m.pattern.0,
            m.match_data.get(root_pos).unwrap()
        );
    }
}
