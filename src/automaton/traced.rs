...
use crate::utils::{TracedNode, Tracer};
...
pub struct TracedAutomaton<'m, C, I> {
    ...
    tracer: Tracer<StateID, TransitionID, Option<C>>,
}
...
impl<'m, C: Eq + Clone, I> TracedAutomaton<'m, C, I> {
    ...
    pub fn apply_trace(self) {
        ...
        for (from, to, constraint) in tracer.into_edge_additions() {
    ...
    fn add_transition_known_child(
        ...
        match self.find_constraint(parent, constraint.as_ref()) {
            ...
            None => {
                // Add edge.
                self.tracer.add_edge(constraint, parent, child);
            }
        }
    }
